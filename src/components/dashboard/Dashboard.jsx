...
const StatsGraphCard = ({title, count, men, women}) => {
  ...
  return(
    ...
    <div className="stats-graph-card">
      ...
      <div className="graph">
        ...
        <path d="M30.5 30.3475C16.7821 33.0562 1.5 51.3475 1.5 51.3475V78.5H116V10.3475C116 10.3475 101.977 8.4406 93 10.3475C76.8958 13.7685 73.7778 31.3817 57.5 33.8475C46.9875 35.4399 40.931 28.2878 30.5 30.3475Z" fill="url(#paint0_linear_5075_132)"/>
        ...
        <defs>
        <linearGradient id="paint0_linear_5075_132" x1="59" y1="-15.5" x2="81" y2="44" gradientUnits="userSpaceOnUse">
        ...
        <p className='percentage'>+2 Past month</p>
...
const Dashboard = () => {
  ...
  const employees = JSON.parse(localStorage.getItem('empDetails'));

  const[date, setDate] = useState(new Date());

  const[filteredData, setFilteredData] = useState([]);

  const maleCount = employees?.filter( employee => employee?.empGender?.toLowerCase() === "male")?.length;  
  
  const femaleCount = employees?.filter( employee => employee?.empGender?.toLowerCase() === "female")?.length;

  useEffect(() => {

    const filteredUserDetails = employees?.filter((data) => data?.empDob?.split(" ")?.slice(0,2).join(" ") == format(new Date(date), "dd MMMM"));
      
    setFilteredData(filteredUserDetails);
    ...
    <div className='dashboard'>
      ...
      <div className="col-stats">
        ...
        <div className="card-wrapper">
          ...
          <StatsCard title={'Leave Request'} count={'0'} style={{backgroundColor: '#FFEFE7'}}/>

          <StatsCard title={'Attendance'} count={employees?.length} style={{backgroundColor: '#E8F0FB'}}/>

          <StatsCard title={'Total Employees'} count={employees?.length} style={{backgroundColor: '#FDEBF9'}}/>

        </div>

        <div className="card-wrapper">

          <StatsGraphCard title={'PayRoll'} count={employees.length} men={maleCount} women={femaleCount}/>

          <StatsGraphCard title={'Task Request'} count={'16'} men={8} women={8}/>
      ...
      <div className="col-calendar">
        ...
        <h3 className='sec-title'>Calendar</h3>

        <MyCalendar date={date} setDate={setDate} />

        <BirthdayCalendar userDetails={filteredData} />
